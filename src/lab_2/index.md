...
```js
const stationInput = Inputs.select(ridership.map((d) => d.station), {unique: true, sort: false, label: "Show me ridership for", width: 200});
const selectedStation = Generators.input(stationInput);
```
...
```js
const selectedStationData = ridership.filter(d => d.station === selectedStation) 
```
